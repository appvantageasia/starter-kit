...
const { flow, map, groupBy } = require('lodash/fp');
...
class WebpackPackagePlugin {
    constructor(options) {
        ...
        const { __metadata, ...entries } = yaml.load(fs.readFileSync(filename, 'utf8'));

        // parse it and get the mapping from it
        this.yarnEntries = processYarnMap(entries);
    }
                ...
                assets => {
                    ...
                    const addDependency = module => {
                        ...
                        if (!builtinModules.includes(module)) {
                            ...
                            externalPackages.add(module);
                        }
                    };

                    compilation.modules.forEach(({ request, externalType }) => {
                        // we only look for external modules
                        let module = externalType && !request.startsWith('./') ? request : null;

                        // detect modules coming from yarn PnP
                        if (/\.yarn/.test(request)) {
                            module = request.match(/.+\/node_modules\/(?<request>.+)$/).groups.request;
                        }

                        if (module) {
                            // get the main module identifier and try to add i
                            addDependency(getParentIdentifier(module));
                        }
                    });
                    ...
                    const [dependencies, yarnMap] = processDependencies(this.yarnEntries, [...externalPackages]);
