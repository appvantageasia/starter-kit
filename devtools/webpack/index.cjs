...
const PackagePlugin = require('./WebpackPackagePlugin.cjs');

const getBabelRule = require('./babel.cjs');
const getStyleRule = require('./style.cjs');
...
const serverConfig = {
    ...
    module: {
        rules: [
            { test: /(antd\/.*?\/style).*(?<![.]js)$/, use: require.resolve('null-loader') },
    ...
    plugins: [
        isBuildIntentProduction && new ForkTsCheckerWebpackPlugin(),
